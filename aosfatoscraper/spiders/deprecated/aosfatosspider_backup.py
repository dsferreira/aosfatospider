...
class AosfatosspiderSpider(scrapy.Spider):
    ...
    def parse_newscheck_page(self, response):
        ...
        # Verifica a condição do link_meta ou "CORREÇÃO" no texto
        if link_meta in hrefs_ultimo or "1." not in text_ultimo:
            ultimo_paragrafo = paragraphs[-2]  # Muda para o penúltimo
            hrefs_ultimo = ultimo_paragrafo.css("a::attr(href)").getall()
            todos_menos_ultimo = response.css(".ck-article p")[1:-2]


        # Verifica a condição do link_metodologia
        if any(link_metodologia in href for href in hrefs_ultimo):
            ultimo_paragrafo = paragraphs[-3]  # Muda para o antepenúltimo
            todos_menos_ultimo = response.css(".ck-article p")[1:-2]  # Reajusta os parágrafos menos o último
